    ...
    for (let j = 0; j < array.length - i - 1; j++) {
      if (array[j] > array[j + 1]) {
        for (let k = 0; k < bars.length; k++) {
          if (k !== j && k !== j + 1) {
            bars[k].style.backgroundColor = "aqua"; //was aqua
          }
        }
        let temp = array[j];
        array[j] = array[j + 1];
...
async function partition(items, left, right) {
  ...
  bars[pivotIndex].style.backgroundColor = "lightgreen"; //was red before

  for (let i = 0; i < bars.length; i++) {
    if (i != pivotIndex) {
      bars[i].style.backgroundColor = "aqua"; //was aqua
    }
  }
...
async function swap(array, i, j, bars) {
  ...
  bars[j].style.height = array[j] * heightFactor + "px";
  bars[i].style.backgroundColor = "lightgreen"; //was red
  bars[j].style.backgroundColor = "red"; //was red
  await sleep(speedFactor);
...
async function mergeSort(arr, start = 0, end = arr.length) {
  ...
  const left = await mergeSort(arr, start, middle);
  const right = await mergeSort(arr, middle, end);
...
// Call the mergeSort function passing your array as an argument
// Example usage:
// const array = [4, 2, 6, 8, 1, 3, 7, 5];
// mergeSort(array);
...
async function selectionSort(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    if (i !== min_idx) {
      await swap(array, i, min_idx, bars);
    }
    bars[i].style.backgroundColor = "lightgreen";
    await sleep(speedFactor);
  }
